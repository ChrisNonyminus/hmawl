...
blacklistedInsns = {
    ...
    PPC_INS_XVCMPGTSP, PPC_INS_XXMRGHD, PPC_INS_XSMADDMDP, PPC_INS_XSMADDADP, PPC_INS_XSMSUBMDP, PPC_INS_XSTDIVDP,
...
def disassemble_insn_that_capstone_cant_handle(o, reloc):
    if reloc:
        relocComment = '\t;# %s:%s' % (
            get_label(reloc['addr']), relocationTypeNames[reloc['type']])
    else:
        relocComment = ''
    raw = read_u32(o)
    asm = None
    ...
    while o < (end & ~3):
        ...
        if reloc:
            ...
            if type == R_PPC_ADDR32:
                ...
                print('dunno what to do about %s here' %
                      relocationTypeNames[type])
        else:
