...
static void patch_rel14_branch_offset(uint8_t *insnp, int32_t branchOffset) {
  const uint32_t offsetMask =
      0x0000FFFC; // bits of instruction that contain the offset
  uint32_t insn = (insnp[0] << 24) // read instruction
                  | (insnp[1] << 16) | (insnp[2] << 8) | (insnp[3] << 0);

  assert(((insn >> 26) & 0x3F) ==
         16); // TODO: do other instructions besides bc use R_PPC_REL14?
  insn =
      (insn & ~offsetMask) | (branchOffset & offsetMask); // patch instruction
  // write instruction
  insnp[0] = (insn >> 24) & 0xFF;
  ...
  for (i = 0, reloc = &import->relocs[0]; i < import->relocsCount;
    ...
    if (reloc->patchSection == sectionId) {
      ...
      if (reloc->type == R_PPC_REL24) {
        ...
        patch_rel14_branch_offset(code + reloc->patchOffset,
                                  reloc->symbolAddr - reloc->patchOffset);
        // remove the relocation
        reloc->type = R_PPC_NONE;
      }
...
static void write_rel_file(struct Module *module, struct RelHeader *relHdr,
  ...
  // In version 3 RELs, this is AFTER the import table, so skip over the import
  // table for now
