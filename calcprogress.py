...
#                                 Description                                  #
################################################################################
# calcprogress: Used to calculate the progress of the HMAWL decompilation.     #
...
DATA_SECTIONS = [
    ...
    ".extab", ".extabindex", "_extab", "_exidx"
]
    ...
    for i in range(first_section, len(symbols)):
        ...
        if (symbols[i].startswith(".") == True or "section layout" in symbols[i]):
            ...
            sectionName = re.search(
                r"\.*(?P<Name>\w+)\s", symbols[i]).group("Name")
            # Determine type of section
            section_type = SECTION_DATA if (
                sectionName in DATA_SECTIONS) else SECTION_TEXT
        # Parse symbols until we hit the next section declaration
            ...
            if "entry of" in symbols[i]:
                ...
                continue
            assert(section_type !=
                   None), f"Symbol found outside of a section!!!\n{symbols[i]}"
            words = symbols[i].split()
            if len(words) == 0:
                continue
            if words[-1].endswith('.s.o'):
                continue
            ...
            j = i
            last_decomped_object = cur_object
            if (section_type == SECTION_TEXT):
                decomp_code_size += cur_size
            else:
                decomp_data_size += cur_size
